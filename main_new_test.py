...
def mainGradientBoostingClassifier( datapath,tag ,savemodelpath ):
    ...
    def parallelrun():
        ...
        # 使用 joblib 并行执行函数
        results = Parallel(n_jobs=12, verbose=0)(delayed(fit_model)(params) for params in tqdm(param_list))
        # results = Parallel(n_jobs=-1, verbose=0)(delayed(fit_model)(params) for params in param_list)
# 
...
def mainxBoostingClassifier2( datapath,tag ,savemodelpath ):
    ...
    # kbins = KBinsDiscretizer(n_bins=10, strategy='uniform',encode='onehot')
    y_binned = kbins.fit_transform(datay.to_numpy() .reshape(-1, 1) ) .squeeze()
    # print('y_train_1',np.unique(y_binned) )
    ...
    # exit()
    # weight = compute_sample_weight('balanced', y_binned)
    X_train, X_test, y_train, y_test = train_test_split(datax,y_binned, test_size=0.01, random_state=randomseed)
    data=[X_train, X_test, y_train, y_test ]
    # weight = compute_sample_weight('balanced', y_train)
    # weight = compute_class_weight(class_weight='balanced', classes= np.unique(y_train) , y=y_train.squeeze() )
    # results = []
...
# 
    ...
    def testrun():
        ...
        for params in  ParameterGrid(param_grid) :
            ...
            model.fit(datax,y_binned )

            # 对新数据进行预测
            y_pred_binned = model.predict(X_train)
            # print('y_pred_binned',y_pred_binned )
            # print('y_test',y_test.shape )
            # accuracy =model.score(y_pred_binned.reshape(-1, 1), y_test.reshape(-1, 1))
            accuracy = accuracy_score(y_train, y_pred_binned)
            result['acc'] = accuracy
            y_train_binned = model.predict(datax)
            train_accuracy = accuracy_score(y_binned, y_train_binned)
            result['train_acc'] = train_accuracy
            for key ,value in params.items():
                result[key] =value
            print(result)
...
def mainxSVC( datapath,tag ,savemodelpath ):
    # C：控制错误样本的惩罚系数，C越大，错误样本的惩罚越大，模型越复杂，可以更好地拟合训练数据，但可能会导致过拟合。

    # kernel：核函数的类型，不同的核函数对应着不同的分类效果和复杂度，可以尝试多种核函数进行比较。

    # gamma：控制核函数的宽度，即影响支持向量的个数和影响区域的大小，gamma越大，影响区域越小，模型越复杂，可以更好地拟合训练数据，但可能会导致过拟合。

    # degree：多项式核函数的阶数，只有在kernel为'poly'时才有用。

    # coef0：核函数中的截距项，只有在kernel为'poly'或'sigmoid'时才有用。

    # class_weight：用于处理不平衡样本的参数，可以设定不同类别的权重。

    # max_iter：最大迭代次数，可以设置模型的最大迭代次数。

    param_grid = {
        'kernel': [ 'linear', 'poly', 'rbf', 'sigmoid'],
        'C' :[  500,1000,5000,10000],
        'gamma': [ 0.0001, 0.01,0.1,1, 100],
        'degree': [10],
    ...
    # exit()
    # weight = compute_sample_weight('balanced', y_binned)
    # weight = compute_class_weight('balanced', y_binned)
    ...
    def testrun():
        ...
        for params in  ParameterGrid(param_grid) :
            result={}
            model =  SVC(**params,class_weight='balanced'  )
            # exit()
            # model.fit(X_train,y_train, sample_weight=weight)
            # print('y_train',np.unique(y_train) )
            model.fit(X_train,y_train )
            # 对新数据进行预测
            y_test_binned = model.predict(X_test)
            accuracy = accuracy_score(y_test, y_test_binned)
            result['acc'] = accuracy
            y_train_binned = model.predict(X_train)
            train_accuracy = accuracy_score(y_train, y_train_binned)
            result['train_acc'] = train_accuracy
            ...
            results.append(result)
        df= pd.DataFrame(results)
        df.to_csv(f'{basename}_{timestamp}_svc.csv',index=False)
        print(df)
        # 获取 age 列最大值所在的行
        # max_age_index = df['acc'].idxmax()
        # max_age_row = df.loc[max_age_index]
...
def mainxboostnew ():
    ...
    mainxSVC( datapath,tag=tag,savemodelpath=savemodelpath)
    # mainGradientBoostingClassifier2(datapath,tag=tag,savemodelpath=savemodelpath)
    # mainxBoostingClassifier2(datapath,tag=tag,savemodelpath=savemodelpath)
