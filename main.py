...
import time
import os
def dataprocess(path,metircs,usearea=False):
    ...
    X_train, X_test, y_train, y_test = train_test_split(data_x, data_y, test_size=0.2, random_state=19960229)
    return X_train, X_test, y_train, y_test
# 加载数据集
def run(path,model,param_grid,metircs='f1s_norm',usearea=False):
    # 梯度提升树
    ...
    X_train, X_test, y_train, y_test = dataprocess(path=path,metircs=metircs,usearea=usearea)
    
    # 网格搜索
    grid_search = GridSearchCV(model, param_grid=param_grid, cv=5, n_jobs=-1, verbose=2,return_train_score=True)
...
# XGBoost  结合 sklearn使用 GridSearchCV
    ...
    if usearea:
        basename = f'{model.__class__}_usearea'.split('.')[-1]
    else:
        basename = f'{model.__class__}_noarea'.split('.')[-1]
...
def maingbt(usearea):
    ...
    gbt =  GradientBoostingRegressor(random_state=19960229)
    # 定义参数空间 把早停放进去
    param_grid = {
...
def mainrf(usearea):
    # path ='/home/dls/data/openmmlab/test_RandomForestRegressor/merge.csv'
    path = '/home/dls/data/openmmlab/test_RandomForestRegressor/merge_all.csv'
    rf =  RandomForestRegressor(random_state=19960229)
    param_grid = {
...
def mainabr(usearea):
    ...
    # random_state：随机数种子，控制随机过程的复现性。
        # path ='/home/dls/data/openmmlab/test_RandomForestRegressor/merge.csv'
    path = '/home/dls/data/openmmlab/test_RandomForestRegressor/merge_all.csv'
    abr =  AdaBoostRegressor(random_state=19960229)
...
def mainxboost(usearea):
    ...
    # }
    metircs='f1s_norm'
    run(path,model=model,param_grid=param_grid,metircs=metircs,usearea=usearea)
